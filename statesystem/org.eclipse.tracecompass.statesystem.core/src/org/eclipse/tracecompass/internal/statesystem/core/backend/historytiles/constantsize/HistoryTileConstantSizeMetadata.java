...
//    private long fPreviousMetadataTile = -1;
    ...
    private Map<Integer, List<Long>> fTileStartTimeMap = new HashMap<>();
    ...
    /**
     * @param writeChannel TODO: Write metadata to file
     */
    public void addTile(FileChannel writeChannel, int detailLevel, long position, long startTime) {
        List<Long> positions = fTilePositionMap.computeIfAbsent(detailLevel, k -> new ArrayList<>());
        List<Long> startTimes = fTileStartTimeMap.computeIfAbsent(detailLevel, k -> new ArrayList<>());
    ...
    public int getLatestSequenceNumber(int detailLevel) {
        return fTileStartTimeMap.getOrDefault(detailLevel, Collections.emptyList()).size();
    }
    ...
    /**
     * @param readChannel TODO: Read metadata from file
     */
    private static Map<Long, Long> readMetadata(FileChannel readChannel) {
        return new HashMap<>();
        //TODO: Write algorithm to read back all the metadata dumps across the history file.
    }

    public int getSequenceNumber(int detailLevel, long timestamp) {
        if (!fTileStartTimeMap.containsKey(detailLevel) || !fTilePositionMap.containsKey(detailLevel)) {
            return -1;
        }
        List<Long> startTimes = fTileStartTimeMap.get(detailLevel);
        int index = Collections.binarySearch(startTimes, timestamp);
    ...
    public long getTilePosition(int detailLevel, int sequenceNumber) {
        ...
        return Objects.requireNonNull(fTilePositionMap.get(detailLevel)).get(sequenceNumber);
    }
}
